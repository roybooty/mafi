...
export const createUser = async (data: User) => {
    try {
        ...
        const newUser = await db.insert(usersTable).values(data).returning({ insertedId: usersTable.id })
...
export const loginUser = async (data: User) => {
    try {
        ...
        const verifyPassword = await argon2.verify(existingUser[0].password, data.password);
        if (!verifyPassword) {
            return { message: "User or Password is incorrect", status: 404 }
        }
